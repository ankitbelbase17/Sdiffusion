"""Official StableVITON trainer wrapper (Unmasked version)."""
...
def train(args):
    ...
    # Wrap underlying PyTorch model with DDP
    ...
    while step < args.max_steps:
        ...
        for batch in loader:
            ...
            # Unmasked processing (User constraints):
            # 1. agnostic = raw initial person image
            # 2. agn_mask = pure black tensor
            # 3. pose = pure black tensor (3 channels for RGB format before VAE encoding)
            agnostic = person
            mask = torch.zeros(person.shape[0], 1, person.shape[2], person.shape[3], device=dist_info.device, dtype=person.dtype)
            pose = torch.zeros_like(person)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Official StableVITON Trainer (Unmasked)")
